...
func openNamespaceState(root, logical string) (*namespaceState, error) {
	...
	rootFD, err := unix.Openat2(unix.AT_FDCWD, absolute, how)
	if err != nil {
...
func (n *namespaceState) revalidate() error {
	...
	if !sameLinuxObject(root, n.rootID) {
		return fmt.Errorf("%w: namespace root identity changed", ErrUnsafe)
	}
	currentFD, err := openLinuxDirectoryPath(n.rootFD, n.logical)
	if err != nil {
	...
	if !sameLinuxObject(current, n.dirID) || !sameLinuxObject(held, n.dirID) {
		return fmt.Errorf("%w: namespace directory identity changed", ErrUnsafe)
	}
	return requireSafeNamespaceDirectory(current)
}

func openLinuxDirectoryPath(rootFD int, logical string) (int, error) {
	parentFD := rootFD
	owned := -1
	for _, part := range strings.Split(logical, "/") {
...
func (n *namespaceState) readRegularBounded(name string, maximum int64) ([]byte, error) {
	...
	if !sameLinuxRegularSnapshot(before, after) || int64(len(content)) != after.Size {
...
func (n *namespaceState) writeRegularAtomic(name string, content []byte, mode fs.FileMode, replace bool) error {
	...
	if replace {
		...
		err = unix.Renameat2(n.dirFD, temporary, n.dirFD, name, unix.RENAME_NOREPLACE)
	}
	if err != nil {
...
func (n *namespaceState) removeRegular(name string) error {
	...
	if err := unix.Renameat2(n.dirFD, name, n.dirFD, quarantine, unix.RENAME_NOREPLACE); err != nil {
		return err
	}
	...
	_ = quarantined.Close()
	if !sameLinuxObject(before, after) || before.Nlink != after.Nlink {
...
func sameLinuxObject(left, right unix.Stat_t) bool {
	return left.Dev == right.Dev && left.Ino == right.Ino
}

func sameLinuxRegularSnapshot(left, right unix.Stat_t) bool {
	return sameLinuxObject(left, right) && left.Mode == right.Mode && left.Nlink == right.Nlink && left.Size == right.Size &&
