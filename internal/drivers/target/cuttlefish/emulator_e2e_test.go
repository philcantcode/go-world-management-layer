...
// then proves normal run finalization fails closed because an externally owned
// AVD cannot provide execution-containment evidence.
func TestAttachedEmulatorDriverEndToEnd(t *testing.T) {
	...
	observedBackend, err := backend.Probe(ctx, ports.TargetTemplate{})
	if err != nil {
		t.Fatalf("observe attached emulator build identity: %v", err)
	}
	...
	files, err := NewCommandFileGateway(CommandFileGatewayConfig{
		ADBBinary: adb, ADBServerEndpoint: DefaultADBServerEndpoint, StagingRoot: filepath.Join(root, "adb-staging"), MaximumTransferBytes: 64 << 20,
	})
	if err != nil {
		t.Fatal(err)
	}
	driver, err := New(Config{
		Build: BuildConfig{
			...
			BackendVersion:    observedBackend.BackendVersion, RuntimeVersion: observedBackend.RuntimeVersion,
	...
	template := ports.TargetTemplate{
		...
		ImageDigest: domain.NewDigest([]byte("api-35-system-image")), IsolationProfile: "attached-e2e", BaselineState: ports.AndroidBaselineCleanBoot,
		RequireHardwareAcceleration: true, Headless: true, Rooted: true, Debuggable: true,
		GuestMemoryBytes: 2 << 30, BootTimeout: 60 * time.Second,
	}
	...
	if prepared.RunID != runPlan.Run.ID() || prepared.Attachment.RuntimeID == "" || prepared.Attachment.RuntimeID == serial ||
		prepared.Attachment.ADBDevice.Serial != serial || prepared.Attachment.ADBDevice.Server != (ports.ADBServerEndpoint{Host: "127.0.0.1", Port: 5037}) {
		t.Fatalf("prepared run did not retain distinct runtime and exact observer ADB authority: %#v", prepared)
	}
	...
	exerciseAndroidSpecimen(t, ctx, adb, serial, apk, proxyServer)
	foreign := "emulator-5556"
	if serial == foreign {
		foreign = "emulator-5558"
	}
	if output, err := runExactADBCommand(ctx, adb, proxyServer, foreign, "get-state"); err == nil {
		t.Fatalf("foreign serial unexpectedly crossed scoped endpoint: %s", output)
	}

	if _, err := driver.StopRun(ctx, runPlan.Run.ID(), ports.StopGraceful); !domain.IsCode(err, domain.CodeCapabilityUnavailable) {
		t.Fatalf("attached emulator stop did not fail closed without containment authority: %v", err)
	}
	if err := transport.Close(); err != nil {
		t.Fatal(err)
	}
	if output, err := runExactADBCommand(ctx, adb, proxyServer, serial, "get-state"); err == nil {
		t.Fatalf("revoked scoped endpoint remained usable: %s", output)
	}
	if output, err := runExactADBCommand(ctx, adb, defaultADBServer, serial, "get-state"); err != nil || strings.TrimSpace(output) != "device" {
		t.Fatalf("attached emulator was stopped instead of detached: %v\n%s", err, output)
	}
}

func exerciseAndroidSpecimen(t *testing.T, ctx context.Context, adb, serial, apk string, server adbServerEndpoint) {
	t.Helper()
	if output, err := runExactADBCommand(ctx, adb, server, serial, "install", "-r", apk); err != nil {
		t.Fatalf("install APK through scoped endpoint: %v\n%s", err, output)
	}
	if output, err := runExactADBCommand(ctx, adb, server, serial, "shell", "am", "start", "-W", "-n", "dev.philcantcode.worldspecimen/.MainActivity"); err != nil || !strings.Contains(output, "Status: ok") {
		t.Fatalf("launch APK through scoped endpoint: %v\n%s", err, output)
	}
	if output, err := runExactADBCommand(ctx, adb, server, serial, "shell", "cmd", "package", "path", "dev.philcantcode.worldspecimen"); err != nil || !strings.Contains(output, "package:") {
		t.Fatalf("query installed APK through scoped endpoint: %v\n%s", err, output)
	}
	reportOutput, err := runExactADBCommand(ctx, adb, server, serial, "shell", "run-as", "dev.philcantcode.worldspecimen", "cat", "files/world-report.json")
	if err != nil {
	...
	if report.Package != "dev.philcantcode.worldspecimen" || report.SDK < 23 || report.Mode != "normal" || report.HostDockerSocketVisible || report.HostWorkspaceVisible {
		t.Fatalf("specimen report violated the Android device boundary: %#v", report)
	}
	if output, err := runExactADBCommand(ctx, adb, server, serial, "shell", "am", "force-stop", "dev.philcantcode.worldspecimen"); err != nil {
		t.Fatalf("stop specimen before crash probe: %v\n%s", err, output)
	}
	if output, err := runExactADBCommand(ctx, adb, server, serial, "logcat", "-c"); err != nil {
		t.Fatalf("clear device log before crash probe: %v\n%s", err, output)
	}
	crashLaunch, crashLaunchErr := runExactADBCommand(ctx, adb, server, serial, "shell", "am", "start", "-W", "-n", "dev.philcantcode.worldspecimen/.MainActivity", "--es", "mode", "crash")
	...
	for time.Now().Before(crashDeadline) {
		crashLog, err = runExactADBCommand(ctx, adb, server, serial, "logcat", "-d", "-t", "400")
	...
	if output, err := runExactADBCommand(ctx, adb, server, serial, "shell", "pidof", "dev.philcantcode.worldspecimen"); err == nil && strings.TrimSpace(output) != "" {
...
func runExactADBCommand(ctx context.Context, adb string, server adbServerEndpoint, serial string, action ...string) (string, error) {
	arguments, err := exactSerialADBArguments(server, serial, action...)
	if err != nil {
		return "", err
	}
	return runADBCommand(ctx, adb, arguments...)
}
...
// adbRemoteShellCommand preserves one sh -c program through adb's second
// remote-shell parse. Passing the raw program as a separate os/exec argument is
// insufficient on Windows: adb joins the remote arguments and sh otherwise
// receives only the first word as its -c program.
func adbRemoteShellCommand(program string) []string {
	return []string{"shell", "sh", "-c", quotePOSIXShellWord(program)}
}
...
func TestADBRemoteShellCommandPreservesOneProgramArgument(t *testing.T) {
	program := `printf '%s' "world managed" > /data/local/tmp/result`
	arguments := adbRemoteShellCommand(program)
	if len(arguments) != 4 || arguments[0] != "shell" || arguments[1] != "sh" || arguments[2] != "-c" {
		t.Fatalf("remote shell arguments = %#v", arguments)
	}
	want := `'printf '"'"'%s'"'"' "world managed" > /data/local/tmp/result'`
	if arguments[3] != want {
		t.Fatalf("quoted remote sh program = %q, want %q", arguments[3], want)
	}
}
